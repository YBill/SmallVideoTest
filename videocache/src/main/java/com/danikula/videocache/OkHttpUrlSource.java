...
public class OkHttpUrlSource implements Source {
    ...
    private long readSourceAvailableBytes(Response response, long offset, int responseCode) throws IOException {
        ...
        return responseCode == HTTP_OK ? contentLength :
                responseCode == HTTP_PARTIAL ?
                        contentLength + offset : sourceInfo.length;
    }

    private long getContentLength(Response response) {
        String contentLengthValue = response.header("Content-Length");
        return contentLengthValue == null ?
                -1 : Long.parseLong(contentLengthValue);
    }
    ...
    private void fetchContentInfo() throws ProxyCacheException {
        ...
        try {
            ...
            this.sourceInfo = new SourceInfo(sourceInfo.url, length, mime);
            this.sourceInfoStorage.put(sourceInfo.url, sourceInfo);
            Logger.info("Content info for `" + sourceInfo.url + "`: mime: " + mime + ", content-length: " + length);
        } catch (IOException e) {
            Logger.error("Error fetching info from " + sourceInfo.url, e);
        } finally {
...
//      okHttpClient.setWriteTimeout(timeout, TimeUnit.MILLISECONDS);
        ...
        do {
            ...
            if (response.isRedirect()) {
                ...
                redirectCount++;
                requestCall.cancel();
                Logger.debug("Redirect closed:" + newUrl);
            }
...
//      okHttpClient.setWriteTimeout(timeout, TimeUnit.MILLISECONDS);
        ...
        int redirectCount = 0;
        do {
            Logger.debug("Open connection " + (offset > 0 ? " with offset " + offset : "") + " to " + sourceInfo.url);
